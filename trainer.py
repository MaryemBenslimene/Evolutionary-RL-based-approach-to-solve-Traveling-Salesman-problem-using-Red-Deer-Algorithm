...
def train_tsp(args):
    ...
    wandb.init(project = 'ERL DRA' , mode = 'online')
    wandb.run.name = 'test ERL RDA with 20 nodes'
    ...
    RDAOptim = RDA.RDA_Optim(population_size=20, num_generations=100, upper_bound=5, lower_bound=-5, gamma=0.5, alpha=0.2, beta=0.1)

    population = RDAOptim.initialize_deer_population (STATIC_SIZE, DYNAMIC_SIZE, args, update_fn)  #Eq (1)
    ...
    RDAOptim.deer_population = RDAOptim.fitness_population(population, critic, test_loader, test_dir, num_plot=5, **kwargs) #Eq (2)
    population, fitness = zip(*RDAOptim.deer_population)
    for fit in fitness :
        wandb.log({"fitness of each individual": fit})

    for generation in range(RDAOptim.num_generations):

        print("Generation :", generation)

        wandb.log({'Generation': generation})

        #forming the male and female deers
        RDAOptim.males_population()
        RDAOptim.hinds_population()

        #roaring of male deer
        print("male roaring")
        RDAOptim.males_roaring(critic, test_loader, test_dir, num_plot=5, **kwargs)


        #forming commanders and stags
        RDAOptim.commanders_population()
        RDAOptim.stags_population()

        #fight between male commanders and stags
        print("commanders stag fight")
        RDAOptim.commanders_stags_fight(critic, test_loader, test_dir, num_plot=5, **kwargs)

        #formation of harems
        print("harem formation")
        harems, num_harems = RDAOptim.harems_formation()

        #mating of commander with hinds in his harem
        print("commanders harem mating")
        offsprings = RDAOptim.commanders_harem_mating(harems, num_harems, critic, test_loader, test_dir, num_plot=5, **kwargs)

        # mating of stag with nearest hind
        print("stag hind mating")
        offsprings = offsprings + RDAOptim.stag_hind_mating(critic, test_loader, test_dir, num_plot=5, **kwargs)

        # selection of the next generation 
        population = RDAOptim.commanders_deer + RDAOptim.stags_deer + RDAOptim.hinds_deer + offsprings
        
        population = sorted(population, key=lambda x: x[1])

        RDAOptim.deer_population = population[:RDAOptim.population_size]
        sorted_pop, fitness = zip(*RDAOptim.deer_population)
        print("fitness", fitness)

        for fit in fitness :
            wandb.log({"fitness of each individual": fit})

        wandb.log({'Mean Fitness': sum(fitness)/RDAOptim.population_size})

    sorted_pop, fitness = zip(*RDAOptim.deer_population)
    print("fitness", fitness)
    actor = RDAOptim.deer_population[0]
    print('Average tour length: ', actor[1])
